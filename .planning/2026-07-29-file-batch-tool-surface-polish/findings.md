...
- Phase C was independently committed as `2303762` (`feat(agent): isolate file batch group commits`); the worktree is clean on `main` and no tag, push, deployment, or release action was taken.
- Phase D is now active under frozen D-11. The first task is an inventory of every standalone Normal/Room descriptor and shared Hub tool surface before changing any wording or schema annotations.
- Descriptor inventory confirms `NORMAL_TOOLS` remains 24 names and Room adds `bootstrap`, `bootstrap.read`, and 10 memory tools for 36 total; the Hub full profile exposes the execution/Room surface plus Hub-native tools, while the coordinator profile exposes 8 Hub-native names.
- The priority descriptor gap was wording rather than dispatch: standalone descriptions omitted the no-shell/read-only boundary, conditional revision/absence guards, per-file `file.batch` partial success, Job follow-up, and the distinction between `mcp.batch` atomic admission and non-transactional downstream side effects. Hub descriptions had the same mcp/job boundary gaps.
- Phase D code changes now repair those descriptions and key property text without adding fields, aliases, or tools; tests assert the contract phrases and existing read-only/destructive annotations.
- A checked-in matrix will use explicit public names grouped only where the same schema/lifecycle applies, with columns for use/non-use, required/conditional inputs, defaults/bounds, failure/lifecycle, and surface parity.
- The first Hub priority-description test exposed that the mcp attribute patch had not reached the exact generated attribute lines; a direct exact-context patch added the intended text, and the test then exposed/verified the bounded-inline-wait wording. No runtime behavior changed.
- Phase D verification passed `cargo fmt --all -- --check`, `cargo check --workspace`, and `cargo clippy --workspace --all-targets -- -D warnings`.
- Full Hub tests pass 59/59, including descriptor acceptance, coordinator/full profile counts, annotations, and the new priority description test. Full Agent tests run 241 with 237 passing; the same four sandbox-permission-sensitive local-control/supervisor/tunnel tests fail and none touch Phase D contract code.
- The user authorized escalation for the environment-sensitive checks; the complete Agent suite now passes 241/241, so Phase D has no remaining test failures.
- Phase D is complete at the D-11 boundary: matrix and wording changes are additive/documentary, descriptor budgets and exact 24/36 counts remain green, and no aliases or runtime execution semantics were introduced.
- Phase D was independently committed as `1251285` (`feat(agent): audit public tool contracts`); the worktree is clean and no tag, push, deployment, or release action occurred.
- Phase E is now active under frozen D-12. The corpus must exercise the real descriptor/schema/serde/dispatch path, not merely hand-written internal DTOs, and optional model scoring must remain offline/provider-neutral by default.
- Added nine bounded cases covering context overshoot, same-file grouped edits, unrelated missing target, guarded create/overwrite shape, Job discovery, MCP admission wording, missing revision, invalid negative context, and typed missing-server failure. The in-tree test substitutes only a temporary fixture revision and exercises the public descriptor/validation/dispatch path.
