...
- Durable install IDs are now treated as path components at every persisted-record and journal boundary. Recovery skips malformed IDs, record reads reject filename/content mismatches, and commit journals must point exactly to the expected skill target and `.archive/<id>/<installId>` entry.
- Commit-journal recovery must branch on `candidate_committed`: before the candidate rename, an existing target may still be the old package and must not be deleted; after the candidate rename, the candidate is removed and a safely archived package is restored. Unexpected mixed states fail closed and leave the filesystem available for operator inspection.
- Cancellation is implemented as cooperative but genuinely interruptible work: the materialization future is selected against the cancellation flag, which drops in-flight HTTP requests, removes staging, and persists `cancelled`. Confirmation-backed skill sessions use a cancellation-aware callback wait so pending Hub senders do not accumulate after `session.kill`.
- The final workspace baseline is green after the hardening pass: 84 local-agent tests, 50 Hub tests, 5 protocol tests, and protocol doc tests (zero cases) passed; formatting and workspace compilation also pass.

## Local integration session findings

- The requested GitHub `planning-with-files` skill was checked against its `SKILL.md`; its relevant workflow is persistent plan files, session catch-up before work, two-inspection checkpoints, and explicit error/progress logging. The repository's active scoped plan already satisfies that layout.
- This integration pass must use an isolated temporary workspace/configuration and exercise the real Room Agent ↔ Hub boundary rather than only unit-level `InstallManager` calls. The next discovery step is to use the existing CLI help/config flow and Hub route surface to identify the smallest local launch topology.
- The real temporary topology worked with the Hub on loopback and a Room Agent registered under `room-integration`; `/v1/room/skills/*` routed without an input `agentId`, while generic session follow-up still used the existing `agentId` query as designed.
- Installing `OthmanAdi/planning-with-files` from `.codex/skills/planning-with-files` exercised GitHub default-ref resolution, commit pinning (`7c6c6cbb76ebee7c7a7e28a38a08d3ad7d1e0427`), recursive tree materialization, executable-mode preservation, source digest summaries, active-state activation, and scoped `SKILL.md` reads.
- The inline smoke package exercised `skills.run`: an explicitly provided executable script returned inline with `completedInline: true`, and its terminal record remained available through the existing session inspect API. A downloaded GitHub script was not executed because the execution approval layer correctly treated that as untrusted code requiring separate authorization.
- The 30-second `slow-long` session held the shared skill lease while a replacement waited in `waiting_for_target`; cancellation reached `cancelled` without mutating the visible package. The workspace also contains the earlier successful `slow-run` replacement archive under `skills/.archive`, demonstrating archive preservation independently of the cancelled replacement.
- Temporary services were stopped cleanly and the exact isolated root `/tmp/agentic-skills-integration-J434A5` was removed after capturing evidence; no repository files were changed by the runtime itself.
