...
- Session catchup reported no unsynced prior implementation context. The product worktree is clean at `5eb6755` (`Add durable Job history store`); only the active plan is tracked-dirty, plus the pre-existing unified-TUI planning folder and `experimental/chrome-control-poc` are untracked.
- Phase 3A and 3B are present in HEAD. `job_history.rs` is independently implemented and tested, but `jobs.rs` still owns the live-only `ManagedJob` lifecycle and does not yet call history admission/terminal persistence or merge history in runtime queries.
- Phase 3C must therefore integrate around existing `jobs.rs` state transitions and preserve the frozen Phase 3D response work for later; no Hub/TUI/chrome-control changes are in scope.
- `ManagedJob` is the single live source in `jobs.rs`; all process/skill/MCP admission paths insert there, while `job_history` currently has no runtime callers. `JobInfo.started_at` is still incorrectly set at admission for process and MCP jobs.
- Actual process start is the successful child-spawn transition in `run_async_job`; queued batch jobs reach the same transition later. Actual MCP start is the successful downstream request attachment in `attach_mcp_request`; confirmation/queue/client setup must not fabricate a start.
- Terminalization is centralized through `finish_job`, `finish_mcp_error`, `complete_mcp_result`, process refresh, and cancellation branches. `finalize_job` currently only writes audit/reporting and consumes its audit context, so history persistence must be a separate idempotent terminal snapshot step to preserve later cancellation/result mutations.
- Existing live retention is 24 hours/100 by `updated_at`; Phase 3C needs a 5-minute/100 hot cache with pending terminal snapshots retained until the bounded history queue either persists or drops them. Existing history list pagination is persisted-only, so runtime needs a global `(createdAt DESC, jobId DESC)` merge cursor and live-wins deduplication.

### Phase 3C implementation findings

- A terminal snapshot is keyed to the live `updatedAt` revision: repeated refreshes do not rewrite history, while post-terminal cancellation metadata gets a new upsert. Pending terminal snapshots are replaced by Job id rather than duplicated.
