...
- Preserve `process.batchExec` as a separate bounded short-command batch operation.
...
Behavior:
1. Resolve policy/path/preflight exactly once.
2. Allocate a managed `sessionId` and register lifecycle state before asynchronous confirmation/spawn.
3. Wait up to `waitSeconds` for a terminal state.
4. When terminal within the wait, return `completedInline=true` and final bounded output.
5. When still `starting`, `waiting_confirmation`, or `running`, return `completedInline=false`, `pollAfterMs=1000`, `sessionId`, and the current session snapshot.
6. Reaching the inline wait never kills the process and is not reported as an execution timeout.
7. Active process capacity uses the existing `limits.maxActiveSessions` boundary; default remains 4 unless a separately evidenced configuration change is required.
8. Terminal records retain the existing 24-hour / newest-100 managed-session policy.

Public response shape is shared with `skills.run`:
- `agentId` — result metadata only; never caller routing.
- `sessionId`
- `completedInline`
- `pollAfterMs`
...
- Unknown ids return the structured existing not-found error.
...
### `process.list`
- Takes no arguments.
- Returns active `starting`, `waiting_confirmation`, and `running` managed processes only, matching existing `session.list` behavior.
- Retained terminal processes remain addressable through `process.get` by known id.

### `process.batchExec`
- Remains a bounded deterministic short-command batch, not an asynchronous batch-session API.
- Input keeps `elements`, `workingDirectory`, and `needConfirm`; removes `agentId` and `confirmMethod` from Tunnel schema.
- Existing all-or-reject preflight, bounded concurrency, result ordering, and batch confirmation semantics remain.
- Global timeout must not leave orphan child processes.
...
### `mcp.list`
- Optional `serverId`.
- Omitted: return configured downstream MCP servers.
- Present: return tools for that configured server.
- No `agentId`.
...
- One stable response envelope is used across all modes; no untyped union response.
...
Behavior:
- `list` delegates to current list-sessions behavior.
- `create` retains path policy, idempotent existing-session result, and no implicit shell command execution.
- `close` retains confirmation and audit behavior.
...
| D-04 | Replace Tunnel `session.*` with `process.exec/get/kill/list`; keep `process.batchExec`. | confirmed |
| D-05 | `process.exec` starts managed immediately, waits default 5/max 30 seconds, and never kills on inline wait expiry. | confirmed |
...
| D-09 | Merge skills list/search/active and activate/deactivate while preserving stale-active semantics. | confirmed |
...
5. Ensure terminal transition audit, lifecycle log, skill lease release, and reporting snapshot happen once.
6. Preserve current Hub session behavior and protocol variants.

**Tests / acceptance:**
- Quick exit, nonzero exit, spawn failure, preflight rejection, confirmation allow/deny/timeout, cancellation before spawn, active-limit rejection, long-running terminal transition, audit exactly once, terminal log exactly once, session reporting best effort, skill lease release.
...
4. Preserve bounded batch semantics and eliminate orphan children on global timeout.
...
- Quick inline, wait-expired running, later get, bounded get wait, kill, list, not found, caller `agentId` rejection, caller `confirmMethod` rejection, configured confirmation provider, batch ordering and no orphan.
...
2. Add one terminal managed-process log for asynchronous completions.
...
7. `process.batchExec` remains ordered/bounded and leaves no orphan children at timeout.
