...
| `file.read` | Read UTF-8 content/metadata; no shell, search process, or write. | `path`; optional `includeContent`, inclusive `startLine`/`endLine`. | Content and line output are bounded; `includeContent` defaults true. | Typed path/UTF-8/size errors; retry-safe and non-destructive. | Normal + Room; same file schema in both ingress paths. |
| `file.search` | In-process literal/regex search; no shell or external search fallback. | `path`, `query`; optional mode/globs/context/limits. | Literal, case-sensitive, Git-ignore aware; context default 0 and live max default 5 (0–100). Overshoots clip with requested/effective evidence. | Invalid regex/glob/path or typed argument errors; read-only and bounded. | Normal + Room; same search envelope as batch search. |
| `file.edit` | One guarded UTF-8 replace, patch, or write; no generic upsert. | `mode`, `path`; existing target needs `expectedRevision`; new target needs `expectedAbsent:true` and `write` content. `replace` needs `oldText`/`newText`; `patch` needs a standard hunk; `write` needs `content`. | `expectedMatches` defaults 1; `dryRun`/`needConfirm` default false. | Revision/absence/path/match/patch/size failures write nothing; one target has atomic replace/no-replace commit and redacted audit. | Normal + Room; standalone only. |
| `file.batch` | Ordered bounded read/search/edit round trip; reads/searches use the pre-edit snapshot and edits commit per normalized file group. No cross-file rollback or `atomicity` selector. | `operations` (1–32; at most 16 edits); per-variant fields; top-level `dryRun`/`needConfirm`. | One aggregate confirmation for valid effective groups; one candidate/stage/commit per normalized target. | Failed read/search/group is isolated; mixed result is `completed_with_errors`; denial writes none; ordered operation + bounded group evidence. | Normal + Room; standalone only. |
