...
mod tests {
    ...
    fn rejects_a_second_lock_and_releases_on_drop() {
        let root = std::env::temp_dir().join(format!("agentic-lock-test-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&root).unwrap();
        let resource = root.join("config.json");
        std::fs::write(&resource, "{}").unwrap();

        let first = InstanceLock::acquire(&resource, ".run.lock", "agent").unwrap();
        let error = InstanceLock::acquire(&resource, ".run.lock", "agent")
            .err()
            .unwrap();
        assert!(error.to_string().contains("already running"));
        drop(first);
        InstanceLock::acquire(&resource, ".run.lock", "agent").unwrap();
